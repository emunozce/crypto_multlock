...
def generate_key(seed, length=16):
    ...
    return key # If seed == 42 then key == OhbVrpoiVgRV5IfL

def polyalphabetic_encrypt(plaintext, key):
    # key is OhbVrpoiVgRV5IfL
...
def main():
    ...
    for _ in range(2):
        key_seed, xor_key = get_timestamp_based_keys() # key_seed == 42 y xor_key == random.randint(1, 255)
        key = generate_key(key_seed) # key == OhbVrpoiVgRV5IfL
        ...
        time.sleep(1) # This line ensures that if the first time the loop runs the timestamp turned to be an odd number, in the sencond run it will be an odd number too.
